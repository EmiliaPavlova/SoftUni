...
select c1.CountryName as [Home Team], c2.CountryName as [Away Team], im.MatchDate as [Match Date] from InternationalMatches im
join Countries c1 on c1.CountryCode = im.HomeCountryCode
join Countries c2 on c2.CountryCode = im.AwayCountryCode
order by [Match Date] desc

-- Problem 6.	*Teams with their League and League Country
select t.TeamName as [Team Name], l.LeagueName as League, (case 
	when l.CountryCode is null then 'International'
	else c.CountryName end) as [League Country] from Teams t
join Leagues_Teams lg on lg.TeamId = t.Id
join Leagues l on l.Id = lg.LeagueId
left join Countries c on c.CountryCode = t.CountryCode
